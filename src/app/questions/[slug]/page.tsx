...
import { getQuestionBySlug, getQuestionPathById, getQuestionSlug } from '@/lib/seo';

const locale = 'zh';

export async function generateStaticParams() {
  return getAllQuestionIds().map((id) => ({ slug: getQuestionSlug(id) }));
}
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return {
    ...
    description,
    alternates: {
      canonical: getCanonicalUrl(getQuestionPathById(question.id)),
    },
  };
}

export default async function SemanticQuestionPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const question = getQuestionBySlug(slug);
    ...
    <div className="space-y-6">
      <Breadcrumb
        items={[
          ...
          { label: getText(question.title, locale) || String(question.id) },
        ]}
      />
          ...
          {(context?.previous || context?.next) && (
            <section className="surface-card p-5">
              ...
              <div className="mt-4 space-y-3">
                {context.previous && (
                  <Link href={getLocalizedPath(locale, getQuestionPathById(context.previous.id))} className="block rounded-[18px] bg-[color:var(--surface-subtle)] px-4 py-3 text-sm transition hover:text-[color:var(--brand-strong)]">
                ...
                {context.next && (
                  <Link href={getLocalizedPath(locale, getQuestionPathById(context.next.id))} className="block rounded-[18px] bg-[color:var(--surface-subtle)] px-4 py-3 text-sm transition hover:text-[color:var(--brand-strong)]">
